...
// var adm = require('./adm');
// var authMiddleware = require('../common/auth_middleware');
var path = require('path');
var tables = require('../controllers/tableController');

//auth API의 경우 하위 라우터에서 인증여부 검사
// router.use('/auth', auth);

// adm API의 경우 인증된 사용자만 접근 가능
// 이 미들웨어가 토큰검증에 실패하면 아래로 next 되지 않는다.
// router.use('/adm', authMiddleware.checkAndDecodeToken); //토큰 체크 및 req.decoded 에 유저정보 생성 미들웨어
// router.use('/adm', adm);

// rewrite virtual urls to angular app to enable refreshing of internal pages
// 위에서 정의되지 않은 모든 url 경로에 대해 index.html 페이지를 쏴준다.
...
// POST파라미터들은 body객체의 하위 데이터로 저장됨.
...
// console.log("req.accepts([types]) : " , req.accepts('json'));
